...
import { EpisodeMissingFieldsError, EpisodeParseError } from "../../errors";
import type {
  ScrapedDownloadHost,
  ScrapedDownloadLink,
  ScrapedEpisode,
...
export type ParsedHost = ScrapedDownloadHost;
export type ParsedDownloadLink = ScrapedDownloadLink;
export type ParsedVideoSource = ScrapedVideoSource;
...
export function mappedEpisodePageToScrapedEpisode(
  parsed: ParsedEpisodePage
): ScrapedEpisode {
  const episodes: ScrapedEpisodeRef[] | undefined = parsed.metadata.episodes?.map(
    (ep) => ({
      title: ep.label,
      url: ep.url,
    })
  ...
  return {
    ...
    posterUrl: parsed.metadata.posterUrl,
    animePageUrl: parsed.metadata.animePageUrl,
    downloadLinks: parsed.metadata.downloadLinks,
    episodes,
    providerData: {
