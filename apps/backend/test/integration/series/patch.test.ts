...
async function insertTestSeries(overrides?: Partial<{
  ...
  const sourceUrl =
    overrides?.sourceUrl ??
    `https://otakudesu.blog/anime/series-patch-test-${crypto.randomUUID()}/`;
...
describe("PATCH /series/:id", () => {
  ...
  describe("partial update behavior", () => {
    ...
    it("successfully updates all allowed fields simultaneously (title, description, posterUrl)", async () => {
      const { accessToken } = await registerUser(app);
      const seriesRow = await insertTestSeries();
